...
def render_invoice(pdf_bytes, company_name, website, addr1, addr2, client_name, loc_line1, loc_line2, remit_email):
    ...
    # Erase only the email address line — the label is in the template and stays
    ...
    edit_zones = [
        ...
        fitz.Rect(x0, email_y0, x1, email_y1),             # email address line only
    ]
    ...
    # Replace only the email address — the label "Email Remittance Advice to:" stays from the template
    if remit_email:
